...
@class TCAsyncSocket;
@class TCAsyncReadPacket;
@class TCAsyncWritePacket;

extern NSString *const TCAsyncSocketException;
extern NSString *const TCAsyncSocketErrorDomain;

enum TCAsyncSocketError
{
	TCAsyncSocketCFSocketError = kCFSocketError,	// From CFSocketError enum.
	TCAsyncSocketNoError = 0,						// Never used.
	TCAsyncSocketCanceledError,					// onSocketWillConnect: returned NO.
	TCAsyncSocketConnectTimeoutError,
	TCAsyncSocketReadMaxedOutError,               // Reached set maxLength without completing
	TCAsyncSocketReadTimeoutError,
	TCAsyncSocketWriteTimeoutError
};
typedef enum TCAsyncSocketError TCAsyncSocketError;

@protocol TCAsyncSocketDelegate
@optional
...
**/
- (void)onSocket:(TCAsyncSocket *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (NSRunLoop *)onSocket:(TCAsyncSocket *)sock wantsRunLoopForNewSocket:(TCAsyncSocket *)newSocket;

/**
 * Called when a socket is about to connect. This method should return YES to continue, or NO to abort.
 * If aborted, will result in TCAsyncSocketCanceledError.
 * 
...
**/
- (void)onSocket:(TCAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(TCAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(TCAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(TCAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(TCAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(TCAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(TCAsyncSocket *)sock;
...
@interface TCAsyncSocket : NSObject
{
	...
	TCAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	TCAsyncWritePacket *theCurrentWrite;
...
// Once one of the accept or connect methods are called, the TCAsyncSocket instance is locked in
...
// When an incoming connection is accepted, TCAsyncSocket invokes several delegate methods.
 ...
 * When a connection comes in, the TCAsyncSocket instance will call the various delegate methods (see above).
 ...
 * 
 * Please note the recommended way of releasing an TCAsyncSocket instance (e.g. in a dealloc method)
 ...
 * 
 * If you pass a buffer, you must not alter it in any way while TCAsyncSocket is using it.
 ...
 * it is treated similarly to a timeout - the socket is closed with a TCAsyncSocketReadMaxedOutError.
 ...
 * The performance can be much improved by allowing TCAsyncSocket to read larger chunks at a time and
...
/**
 * When you create an TCAsyncSocket, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the TCAsyncSocket instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
