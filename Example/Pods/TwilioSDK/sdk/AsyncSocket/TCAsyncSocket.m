...
// TCAsyncSocket is RunLoop based, and is thus not thread-safe.
// You must always access your TCAsyncSocket instance from the thread/runloop in which the instance is running.
...
NSString *const TCAsyncSocketException = @"TCAsyncSocketException";
NSString *const TCAsyncSocketErrorDomain = @"TCAsyncSocketErrorDomain";


enum TCAsyncSocketFlags
{
	kTCEnablePreBuffering      = 1 <<  0,  // If set, pre-buffering is enabled
	kTCDidStartDelegate        = 1 <<  1,  // If set, disconnection results in delegate call
	kTCDidCompleteOpenForRead  = 1 <<  2,  // If set, open callback has been called for read stream
	kTCDidCompleteOpenForWrite = 1 <<  3,  // If set, open callback has been called for write stream
	kTCStartingReadTLS         = 1 <<  4,  // If set, we're waiting for TLS negotiation to complete
	kTCStartingWriteTLS        = 1 <<  5,  // If set, we're waiting for TLS negotiation to complete
	kTCForbidReadsWrites       = 1 <<  6,  // If set, no new reads or writes are allowed
	kTCDisconnectAfterReads    = 1 <<  7,  // If set, disconnect after no more reads are queued
	kTCDisconnectAfterWrites   = 1 <<  8,  // If set, disconnect after no more writes are queued
	kTCClosingWithError        = 1 <<  9,  // If set, the socket is being closed due to an error
	kTCDequeueReadScheduled    = 1 << 10,  // If set, a maybeDequeueRead operation is already scheduled
	kTCDequeueWriteScheduled   = 1 << 11,  // If set, a maybeDequeueWrite operation is already scheduled
	kTCSocketCanAcceptBytes    = 1 << 12,  // If set, we know socket can accept bytes. If unset, it's unknown.
	kTCSocketHasBytesAvailable = 1 << 13,  // If set, we know socket has bytes available. If unset, it's unknown.
};

@interface TCAsyncSocket (Private)
...
/**
 * The TCAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface TCAsyncReadPacket : NSObject
{
  @public
...
@implementation TCAsyncReadPacket
...
/**
 * The TCAsyncWritePacket encompasses the instructions for any given write.
**/
@interface TCAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation TCAsyncWritePacket
...
/**
 * The TCAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface TCAsyncSpecialPacket : NSObject
{
  @public
...
@implementation TCAsyncSpecialPacket
...
- (id)initWithDelegate:(id)delegate userData:(long)userData
{
	if((self = [super init]))
	{
		theFlags = DEFAULT_PREBUFFERING ? kTCEnablePreBuffering : 0;
...
- (void)checkForThreadSafety
{
	...
	{
		// TCAsyncSocket is RunLoop based.
		...
		// If you are seeing errors or crashes in TCAsyncSocket,
		...
		[NSException raise:TCAsyncSocketException
		            format:@"Attempting to access TCAsyncSocket instance from incorrect thread."];
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	// and that the read packet isn't an TCAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[TCAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	// and that the write packet isn't an TCAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[TCAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (void)enablePreBuffering
{
	...
	theFlags |= kTCEnablePreBuffering;
}
...
- (BOOL)moveToRunLoop:(NSRunLoop *)runLoop
{
	...
	[NSObject cancelPreviousPerformRequestsWithTarget:self];
	theFlags &= ~kTCDequeueReadScheduled;
	theFlags &= ~kTCDequeueWriteScheduled;
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	...
	if(port == 0 && theSocket4 && theSocket6)
	{
        ...
        if (pSockAddr6 == NULL) goto Failed;
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	else
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketCFSocketError userInfo:info];
		}
		return NO;
	}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		// Note: We use [self class] to support subclassing TCAsyncSocket.
		TCAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
		...
		newSocket->theFlags |= kTCDidStartDelegate;
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		NSLog(@"TCAsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"TCAsyncSocket %p couldn't attach read stream to run-loop,", self);
		NSLog (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"TCAsyncSocket %p couldn't attach write stream to run-loop,", self);
		NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
  ...
  CFWriteStreamSetProperty(theWriteStream, kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP); 
	...
	{
		NSLog (@"TCAsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
		NSLog (@"TCAsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the TCAsyncSocket instance to be ready.
**/
- (void)doStreamOpen
{
	if ((theFlags & kTCDidCompleteOpenForRead) && (theFlags & kTCDidCompleteOpenForWrite))
	{
		NSError *err = nil;
		...
		{
			NSLog (@"TCAsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		NSLog(@"TCAsyncSocket couldn't determine IP version of socket");
...
- (void)closeWithError:(NSError *)err
{
	theFlags |= kTCClosingWithError;
	
	if (theFlags & kTCDidStartDelegate)
	{
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		// Check to see if it's a normal read packet (not TCAsyncSpecialPacket) and if it had read anything yet.
		
		if(([theCurrentRead isKindOfClass:[TCAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (void)close
{
	...
	BOOL shouldCallDelegate = (theFlags & kTCDidStartDelegate);
...
- (void)disconnectAfterReading
{
	...
	theFlags |= (kTCForbidReadsWrites | kTCDisconnectAfterReads);
...
- (void)disconnectAfterWriting
{
	...
	theFlags |= (kTCForbidReadsWrites | kTCDisconnectAfterWrites);
...
- (void)disconnectAfterReadingAndWriting
{
	...
	theFlags |= (kTCForbidReadsWrites | kTCDisconnectAfterReads | kTCDisconnectAfterWrites);
...
- (void)maybeScheduleDisconnect
{
	...
	if(theFlags & kTCDisconnectAfterReads)
	{
		if(([theReadQueue count] == 0) && (theCurrentRead == nil))
		{
			if(theFlags & kTCDisconnectAfterWrites)
			{
	...
	else if(theFlags & kTCDisconnectAfterWrites)
	{
		if(([theWriteQueue count] == 0) && (theCurrentWrite == nil))
		{
			shouldDisconnect = YES;
...
- (NSData *)unreadData
{
	...
	if (!(theFlags & kTCClosingWithError)) return nil;
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"TCAsyncSocketCFSocketError",
														 @"TCAsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketCFSocketError userInfo:info];
}
...
/**
 * Returns a standard TCAsyncSocket abort error.
**/
- (NSError *)getAbortError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"TCAsyncSocketCanceledError",
														 @"TCAsyncSocket", [NSBundle mainBundle],
														 @"Connection canceled", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketCanceledError userInfo:info];
}

/**
 * Returns a standard TCAsyncSocket connect timeout error.
**/
- (NSError *)getConnectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"TCAsyncSocketConnectTimeoutError",
														 @"TCAsyncSocket", [NSBundle mainBundle],
														 @"Attempt to connect to host timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketConnectTimeoutError userInfo:info];
}

/**
 * Returns a standard TCAsyncSocket maxed out error.
**/
- (NSError *)getReadMaxedOutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"TCAsyncSocketReadMaxedOutError",
														 @"TCAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketReadMaxedOutError userInfo:info];
}

/**
 * Returns a standard TCAsyncSocket read timeout error.
**/
- (NSError *)getReadTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"TCAsyncSocketReadTimeoutError",
														 @"TCAsyncSocket", [NSBundle mainBundle],
														 @"Read operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketReadTimeoutError userInfo:info];
}

/**
 * Returns a standard TCAsyncSocket write timeout error.
**/
- (NSError *)getWriteTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"TCAsyncSocketWriteTimeoutError",
														 @"TCAsyncSocket", [NSBundle mainBundle],
														 @"Write operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:TCAsyncSocketErrorDomain code:TCAsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	[ms appendString:[NSString stringWithFormat:@"<TCAsyncSocket %p", self]];
                        ...
                        tag:(long)tag
{
	...
	if (theFlags & kTCForbidReadsWrites) return;
	
	TCAsyncReadPacket *packet = [[TCAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)scheduleDequeueRead
{
	if((theFlags & kTCDequeueReadScheduled) == 0)
	{
		theFlags |= kTCDequeueReadScheduled;
		[self performSelector:@selector(maybeDequeueRead) withObject:nil afterDelay:0 inModes:theRunLoopModes];
	}
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[TCAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
				theFlags |= kTCStartingReadTLS;
				
				// This method won't do anything unless both kTCStartingReadTLS and kTCStartingWriteTLS are set
				[self maybeStartTLS];
			}
			else
			{
		...
		else if(theFlags & kTCDisconnectAfterReads)
		{
			if(theFlags & kTCDisconnectAfterWrites)
			{
				if(([theWriteQueue count] == 0) && (theCurrentWrite == nil))
 ...
 * This method supports pre-buffering properly as well as the kTCSocketHasBytesAvailable flag.
**/
- (BOOL)hasBytesAvailable
{
	if ((theFlags & kTCSocketHasBytesAvailable) || ([partialReadBuffer length] > 0))
	{
		return YES;
	}
	else
...
- (CFIndex)readIntoBuffer:(void *)buffer maxLength:(NSUInteger)length
{
	...
	else
	{
		...
		theFlags &= ~kTCSocketHasBytesAvailable;
...
- (void)doBytesAvailable
{
	...
	// Note: This method is not called if theCurrentRead is an TCAsyncSpecialPacket (startTLS packet)
	...
	while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
	{
		...
		if (theCurrentRead->term != nil)
		{
			...
			{
				if (theFlags & kTCEnablePreBuffering)
				{
					didPreBuffer = YES;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	TCAsyncWritePacket *packet = [[TCAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)scheduleDequeueWrite
{
	if((theFlags & kTCDequeueWriteScheduled) == 0)
	{
		theFlags |= kTCDequeueWriteScheduled;
		[self performSelector:@selector(maybeDequeueWrite) withObject:nil afterDelay:0 inModes:theRunLoopModes];
	}
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[TCAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
				theFlags |= kTCStartingWriteTLS;
 ...
 * This method supports the kTCSocketCanAcceptBytes flag.
**/
- (BOOL)canAcceptBytes
{
	if (theFlags & kTCSocketCanAcceptBytes)
	{
		return YES;
	}
	else
...
- (void)doSendBytes
{
	...
	// Note: This method is not called if theCurrentWrite is an TCAsyncSpecialPacket (startTLS packet)
	...
	while (!done && !error && [self canAcceptBytes])
	{
		...
		theFlags &= ~kTCSocketCanAcceptBytes;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	TCAsyncSpecialPacket *packet = [[TCAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	// 
	// We'll know these conditions are met when both kTCStartingReadTLS and kTCStartingWriteTLS are set
	
	if((theFlags & kTCStartingReadTLS) && (theFlags & kTCStartingWriteTLS))
	{
		TCAsyncSpecialPacket *tlsPacket = (TCAsyncSpecialPacket *)theCurrentRead;
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog(@"TCAsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		case kCFStreamEventOpenCompleted:
			theFlags |= kTCDidCompleteOpenForRead;
			[self doStreamOpen];
			break;
		case kCFStreamEventHasBytesAvailable:
			if(theFlags & kTCStartingReadTLS) {
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog(@"TCAsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	{
		case kCFStreamEventOpenCompleted:
			theFlags |= kTCDidCompleteOpenForWrite;
			[self doStreamOpen];
			break;
		case kCFStreamEventCanAcceptBytes:
			if(theFlags & kTCStartingWriteTLS) {
				[self onTLSHandshakeSuccessful];
			}
			else {
				theFlags |= kTCSocketCanAcceptBytes;
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog(@"TCAsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
{
    @autoreleasepool {
        TCAsyncSocket *theSocket = [[(TCAsyncSocket *)pInfo retain] autorelease];
