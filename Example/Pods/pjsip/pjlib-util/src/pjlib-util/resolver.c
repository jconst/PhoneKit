...
#ifdef PJ_IS_IOS
    DNSServiceRef        sdRef;		/**< iOS DNS query pointer	    */
    pj_ioqueue_key_t	*sdRef_key;	/**< iOS UDP socket ioqueue key.    */
    pj_ioqueue_op_key_t	 sdRef_op_key;	/**< iOS UDP read operation key.    */
    pj_status_t          dns_status;    /**< iOS passes errcode back async. */
#endif
};
 ...
 */
PJ_DEF(pj_status_t) pj_dns_resolver_flush_cache( pj_dns_resolver *resolver)
{
	    ...
	    const char *name)
{
    ...
    // DNS names are broken down into components separated
    // by a dot.  the format in the packet consists of a
    // single byte for length, then the component, byte
    // for length, component, etc., until we get to the end,
    // where we append a final length byte of 0x00
	...
	buf[written++] = end - start;
	memcpy(buf + written, start, end - start);
	written += end - start;
	start = end + 1;
    } while (*end == '.');
...
static void
ios_dns_service_callback(DNSServiceRef sdRef,
                         ...
                         void *context)
{
    pj_dns_async_query *q = (pj_dns_async_query *)context;
    pj_dns_resolver *resolver = q->resolver;

    if (errorCode != kDNSServiceErr_NoError) {
	// FIXME: descriptive error codes
        PJ_LOG(2, (resolver->name.ptr, "DNSServices callback gave us error %d", errorCode));
        q->dns_status = PJ_EUNKNOWN;
	return;
    }

    // annoyingly we only get part of the answer section of the response, but
    // pj_dns_parse_packet() expects the full response data, so let's fake it

    uint8_t *buf = (uint8_t *)resolver->udp_rx_pkt;
    size_t off = resolver->udp_len;
    size_t space_left = sizeof(resolver->udp_rx_pkt) - off;
    size_t space_needed;

    ssize_t written;

    uint16_t qtype_net = pj_htons(q->key.qtype);
    uint16_t rrtype_net = pj_htons(rrtype);
    uint16_t rrclass_net = pj_htons(rrclass);
    uint16_t rdlen_net = pj_htons(rdlen);
    uint32_t ttl_net = pj_htonl(ttl);

    if (off == 0) {
	// first time the callback was called for us

        // make sure we have enough space
        // DNS packet header
        space_needed = sizeof(pj_dns_hdr);
        // question name length byte + name + NUL term
        space_needed += 1 + strlen(q->key.name) + 1;
        // query type + query class
        space_needed += sizeof(qtype_net) + sizeof(rrclass_net);

        if (space_needed > space_left) {
            PJ_LOG(2, (resolver->name.ptr, "DNS reply is too large for packet buffer"));
            q->dns_status = PJ_ENOMEM;
            return;
        }

	pj_dns_hdr h;
	h.id = pj_htons(q->id);
	h.flags = pj_htons(PJ_DNS_SET_QR(1) | PJ_DNS_SET_RA(1));
	h.qdcount = pj_htons(1);
	h.anscount = pj_htons(1);
	h.nscount = pj_htons(0);
	h.arcount = pj_htons(0);

	// copy in header
	memcpy(buf + off, &h, sizeof(h));
	off += sizeof(h);

	// fake the question section
	written = append_name(buf + off, q->key.name);
	if (written == -1) {
            q->dns_status = PJ_EUNKNOWN;
	    return;
        }
	off += written;
	memcpy(buf + off, &qtype_net, sizeof(qtype_net));
	off += sizeof(qtype_net);
	memcpy(buf + off, &rrclass_net, sizeof(rrclass_net));
	off += sizeof(rrclass_net);

        // recalc
        space_left = sizeof(resolver->udp_rx_pkt) - off;
    } else {
	// successive RRs, just increment 'anscount'
	uint16_t *anscount = (uint16_t *)(buf + offsetof(pj_dns_hdr, anscount));
	*anscount = pj_htons(pj_ntohs(*anscount) + 1);
    }

    // update in case we bail below
    resolver->udp_len = off;

    // space check
    // record name length byte + record name + NUL term
    space_needed = 1 + strlen(q->key.name) + 1;
    // record type + record class + ttl + data length
    space_needed += sizeof(rrtype_net) + sizeof(rrclass_net) + sizeof(ttl_net) + sizeof(rdlen_net);
    // record data
    space_needed += rdlen;

    if (space_needed > space_left) {
        // we'll at least try to return a truncated reply if
        // we have at least one answer RR
	uint16_t *anscount = (uint16_t *)(buf + offsetof(pj_dns_hdr, anscount));
	*anscount = pj_htons(pj_ntohs(*anscount) - 1);

        if (*anscount == 0) {
            PJ_LOG(2, (resolver->name.ptr, "DNS reply is too large for packet buffer; no records fit"));
            q->dns_status = PJ_ENOMEM;
        } else {
            PJ_LOG(3, (resolver->name.ptr, "DNS reply is too large; truncating"));
            uint16_t *flags = (uint16_t *)(buf + offsetof(pj_dns_hdr, flags));
            *flags = pj_htons(pj_ntohs(*flags) | PJ_DNS_SET_TC(1));
        }
    ...
    off += written;
    memcpy(buf + off, &rrtype_net, sizeof(rrtype_net));
    off += sizeof(rrtype_net);
    memcpy(buf + off, &rrclass_net, sizeof(rrclass_net));
    off += sizeof(rrclass_net);
    memcpy(buf + off, &ttl_net, sizeof(ttl_net));
    off += sizeof(ttl_net);
    memcpy(buf + off, &rdlen_net, sizeof(rdlen_net));
    off += sizeof(rdlen_net);
...
static void
ios_release_query(pj_dns_async_query *q)
{
    q->sdRef_op_key.user_data = NULL;

    if (q->sdRef_key) {
        pj_ioqueue_unregister(q->sdRef_key);
        q->sdRef_key = NULL;
    }

    if (q->sdRef) {
        DNSServiceRefDeallocate(q->sdRef);
        q->sdRef = NULL;
    }
}
				  ...
				  pj_dns_async_query *q)
{
    ...
    PJ_UNUSED_ARG(i);
    PJ_UNUSED_ARG(server_cnt);
    PJ_UNUSED_ARG(servers);
    PJ_UNUSED_ARG(now);
    PJ_UNUSED_ARG(name);

    ios_release_query(q);  // just in case
    q->dns_status = PJ_SUCCESS;

    DNSServiceErrorType dns_err = DNSServiceQueryRecord(&q->sdRef, 0, 0, q->key.name,
							q->key.qtype, kDNSServiceClass_IN,
							ios_dns_service_callback, q);
    if (dns_err == kDNSServiceErr_NoError) {
        char errmsg[1024] = "";
	pj_ioqueue_callback socket_cb;
	pj_bzero(&socket_cb, sizeof(socket_cb));
	socket_cb.on_read_complete = &on_read_complete;
	status = pj_ioqueue_register_sock(resolver->pool, resolver->ioqueue,
					  DNSServiceRefSockFD(q->sdRef),
					  resolver, &socket_cb, &q->sdRef_key);
	if (status == PJ_SUCCESS) {
	    pj_ioqueue_op_key_init(&q->sdRef_op_key, sizeof(q->sdRef_op_key));

	    // this is kinda cheating.  we want to make sure the pj_ioqueue
	    // doesn't actually read anything from the socket (else DNSServices
	    // gets very angry with us), so we queue a zero-length read
	    // into a phony buffer.  hopefully this doesn't explode.
	    pj_ssize_t len = 0;
	    void *buf = (void *)0xdeadbeef;
	    status = pj_ioqueue_recv(q->sdRef_key, &q->sdRef_op_key,
					 buf, &len,
					 PJ_IOQUEUE_ALWAYS_ASYNC);
	    if (status == PJ_EPENDING) {
		status = PJ_SUCCESS;
		q->sdRef_op_key.user_data = q;
	    } else {
                pj_strerror(status, errmsg, sizeof(errmsg));
                PJ_LOG(2, (resolver->name.ptr, "Failed to fake recv on DNSServices socket: %s", errmsg));
            }
	} else {
            pj_strerror(status, errmsg, sizeof(errmsg));
            PJ_LOG(2, (resolver->name.ptr, "Failed to register DNSServices socket: %s", errmsg));
        }

        if (status != PJ_SUCCESS)
            ios_release_query(q);
    } else {
        PJ_LOG(2, (resolver->name.ptr, "DNSServiceQueryRecord() returned error %d", dns_err));
	status = PJ_EUNKNOWN;
    }
#endif
...
#ifdef PJ_IS_IOS
    /* tell iOS to try to process the incoming data */
    resolver->udp_len = 0;
    q = (pj_dns_async_query *)op_key->user_data;
    if (!q)
        goto read_next_packet;

    if (DNSServiceProcessResult(q->sdRef) != kDNSServiceErr_NoError)
        q->dns_status = PJ_EUNKNOWN;

    if (q->dns_status == PJ_SUCCESS)
        bytes_read = resolver->udp_len;
    else
        bytes_read = -q->dns_status;
