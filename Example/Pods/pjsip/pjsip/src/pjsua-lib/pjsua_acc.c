				   ...
				   const pjsua_acc_config *src)
{
	...
	while (hdr != &src->reg_hdr_list && (NULL != hdr->name.ptr)) {
	...
	while (hdr != &src->sub_hdr_list && (NULL != hdr->name.ptr)) {
				      ...
				      const pjsua_acc_config *cfg)
{
    ...
    /* REGISTER header list, just copy over the new one, better option to check for modifications */
    pj_list_init(&acc->cfg.reg_hdr_list);
    const pjsip_hdr* src = &cfg->reg_hdr_list;
    const pjsip_hdr* src_i;
    for (src_i = src->next; src_i != src; src_i = src_i->next) {
        pjsip_hdr* h = pjsip_hdr_clone(acc->pool, src_i);
        pj_list_push_back(&acc->cfg.reg_hdr_list, h);
    }
    update_reg = PJ_TRUE;
    unreg_first = PJ_TRUE;
