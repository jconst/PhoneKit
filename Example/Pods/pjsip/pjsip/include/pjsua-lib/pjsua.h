...
/**
 * Get pjsip_transport that matches a given pjsua_transport_id.
 *
 * @param id		Transport ID.
 * @param tp		Pointer to receive transport.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_transport_get(pjsua_transport_id id,
					 pjsip_transport **tp);
...
/**
 * Obtains the call's PJSIP invite session object.
 *
 * @param call_id	Call identification;
 * @param invp		Pointer to inv session to be initialized.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_call_get_inv_session(pjsua_call_id call_id,
						pjsip_inv_session **invp);
