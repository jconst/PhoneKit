...
static void interruptionListener(void *inClientData, UInt32 inInterruption)
{
	...
	{
        /* This is to work around the iOS CoreAudio issue of not
         * actually calling us back when the audio stream is
         * interrupted. Probably because threads aren't being
         * scheduled before the objects are torn down or some
         * other nonsense.
         */
        if (it->stream->interrupted == PJ_FALSE)
        {
            status = ca_stream_stop((pjmedia_aud_stream*)it->stream);
